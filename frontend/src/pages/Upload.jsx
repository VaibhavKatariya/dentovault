...
export default function Upload() {
  ...
  useEffect(() => {
    api.get(`/patients/${id}`).then((r) => setPatient(r.data)).catch(() => navigate("/patients"));
  }, [id, navigate]);
  ...
  return (
    <div className="p-6 sm:p-10 max-w-4xl">
      <Link to={`/patients/${id}`} className="inline-flex items-center gap-2 label-mono mb-6 hover:text-foreground">
      ...
      <div
        ...
        className={`border border-dashed transition-colors cursor-pointer p-12 text-center ${
          dragOver ? "border-foreground bg-accent" : "border-border hover:border-foreground/60"
        }`}
      >
